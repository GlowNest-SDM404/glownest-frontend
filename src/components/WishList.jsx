    ...
    <div className="container wishlist-page">
      ...
      <div className="wishlist-items">
        {wishlist.map((item) => (
          <div key={item.id} className="wishlist-card card">
            {item.inStock && (
              <span className="badge bg-success in-stock">In Stock</span>
            )}
      ...
      {wishlist.length > 0 && (
        <div className="wishlist-actions">
          <button className="btn btn-outline-primary" onClick={moveAllToCart}>
            Move All to Cart
          </button>
          <button className="btn btn-link text-danger" onClick={clearWishlist}>
            Clear Wishlist
          </button>
        </div>
      )}
