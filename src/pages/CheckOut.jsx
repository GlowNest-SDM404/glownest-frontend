...
export default function CheckOut() {
  ...
  const orderTotal = checkoutData.subtotal + checkoutData.shipping + checkoutData.tax;
  ...
  return (
    <div className="checkout-page container">
        ...
        <ul className="checkout-steps">
            <li className={step === 1 ? "active" : step > 1 ? "completed" : ""}>Cart Summary</li>
            <li className={step === 2 ? "active" : step > 2 ? "completed" : ""}>Shipping Information</li>
        ...
        <div className="row mt-4">
            ...
            <div className="section">
                ...
                <div key={item.productId} className="cart-item row">
                    <div className="col-3"><img src={item.image} alt={item.name} className="img-fluid" /></div>
                    <div className="col-7">
                        <strong>{item.name}</strong>
                        <div><span className="text">${item.price.toFixed(2)} × {item.qty}</span></div>
                    </div>
                    <div className="col-2 text-end">${(item.price * item.qty).toFixed(2)}</div>
                </div>
                ))}
                <button className="btn btn-primary continue-btn mt-3 shipping-btn" onClick={() => setStep(2)}>
            ...
            <div className="col-md-4">
                <div className="order-summary card p-3">
                    ...
                    <hr />
                    <div className="total d-flex justify-content-between"><strong>Total</strong><strong>${orderTotal.toFixed(2)}</strong></div>
                    <button className="btn btn-primary continue-btn mt-3 place-order-btn" onClick={handlePlaceOrder}>
                        Place Order
                    </button>
                    <span className="term-text">By placing your order, you agree to GlowNest's Terms of Service.</span>
                </div>
            </div>
        </div>
    </div>
